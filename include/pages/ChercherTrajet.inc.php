<?php
/*Affichage de la page si la personne est connecté sinon retour à la page d'accueil*/
...
/*Affichage de la première ville*/
  ...
  if(!empty($listeTrajet)){
    ...
    <?php
          ...
          ?>
              <tr><td>
                <?php
                if($parcoursManager->getSens($_SESSION["vil_num1"],$_POST["vil_num2"]) == 1){
                  echo $villeManager->getVilleNom($trajet["vil_num2"])["vil_nom"];
                }else{
                  echo $villeManager->getVilleNom($trajet["vil_num1"])["vil_nom"];
                }?>
              </td><td>
                <?php
              if($parcoursManager->getSens($_SESSION["vil_num1"],$_POST["vil_num2"]) == 1){
                ...
                <div id="parent">
                  ...
                  <?php echo $personneManager->RecupPersPrenom($trajet["per_num"])["per_prenom"]," ", $personneManager->RecupPersNom($trajet["per_num"])["per_nom"];?>
                  <div id="enfant" class="habillage">
                    Dernière avis :
                    <?php echo $personneManager->RecupAvis($trajet["per_num"])["avi_comm"]?>
                  </div>
                  </div>
